...
import { VerticalMode, LateralMode, isArmed, ArmedLateralMode, ArmedVerticalMode } from '@shared/autopilot';
import {
...
import { ApproachType, ApproachWaypointDescriptor, MathUtils } from '@flybywiresim/fbw-sdk';
...
import { didVnavGeometryLegsChange, VnavPlanProfileCoherence } from './VnavProfileCoherence';

const DESTINATION_FUEL_RECOMPUTE_THRESHOLD = 100;
const APPROACH_SPEED_RECOMPUTE_THRESHOLD = 1;
...
export class VnavDriver implements GuidanceComponent {
  ...
  private readonly planProfileCoherence = new VnavPlanProfileCoherence();

  /** A generated-FMGC release sample must occur before a replacement profile can take ownership. */
  private planProfileReleasePending: boolean = false;

  /** Set only after the release update has run; consumed by the next replacement-profile rebuild. */
  private planProfileRebuildPermitted: boolean = false;

  /** Incremented at every authoritative active-plan ownership boundary. */
  private activePlanDerivedDataGeneration: number = 0;

  /** The generation for which all plan-dependent MCDU caches have been refreshed. */
  private readyActivePlanDerivedDataGeneration: number = 0;
  ...
  update(deltaTime: number): void {
    SimVar.SetSimVarValue('L:A339X_DEBUG_FINAL_UPDATE_REACHED', 'Bool', false);
    let approachFamily = FmgcApproachFamily.None;

    try {
      const activePlan = this.flightPlanService.hasActive ? this.flightPlanService.active : undefined;
      approachFamily = classifyFmgcApproachFamily(activePlan);
      const { flightPhase } = this.computationParametersObserver.get();

      this.updateDebugInformation();

      const activeGeometry = this.guidanceController.activeGeometry;
      if (this.planProfileReleasePending) {
        this.releaseIncoherentPlanProfile(approachFamily, activePlan !== undefined && activeGeometry != null);
        return;
      }
      ...
      if (
        ...
        activeGeometry == null ||
        !this.planProfileCoherence.isCoherent(activePlan, activeGeometry)
      ) {
        this.releaseIncoherentPlanProfile(approachFamily, activePlan !== undefined && activeGeometry != null);
        return;
      }
      ...
      this.updateFinalApproachGuidance(flightPhase);
    } catch (e) {
      // The generated FMGC samples these inputs together and resets active FINAL
      // before selecting the longitudinal law, so zero targets cannot feed VPATH.
      this.publishFinalApproachGuidance(approachFamily, false, false);
      this.planProfileCoherence.reset();
      this.invalidateIncoherentPlanProfile();
      console.error('[FMS] Failed to calculate vertical profile. See exception below.');
      console.error(e);
    }
  }

  private updateFinalApproachGuidance(flightPhase: FmgcFlightPhase): void {
    SimVar.SetSimVarValue('L:A339X_DEBUG_FINAL_UPDATE_REACHED', 'Bool', true);

    const activePlan = this.flightPlanService.hasActive ? this.flightPlanService.active : undefined;
    const approachType = activePlan?.approach?.type ?? -1;
    const approachFamily = classifyFmgcApproachFamily(activePlan);

    SimVar.SetSimVarValue('L:A339X_DEBUG_APPROACH_PRESENT', 'Bool', activePlan?.approach !== undefined);
    SimVar.SetSimVarValue('L:A339X_DEBUG_APPROACH_ACTIVE', 'Bool', activePlan?.isApproachActive ?? false);
    SimVar.SetSimVarValue('L:A339X_DEBUG_APPROACH_TYPE', 'Number', approachType);
    SimVar.SetSimVarValue(
      'L:A339X_DEBUG_APPROACH_AUTH_REQUIRED',
      'Bool',
      activePlan?.approach?.authorisationRequired === true,
    );
    SimVar.SetSimVarValue('L:A339X_DEBUG_FIRST_APPROACH_LEG_INDEX', 'Number', activePlan?.firstApproachLegIndex ?? -1);
    SimVar.SetSimVarValue('L:A339X_DEBUG_ACTIVE_LEG_INDEX', 'Number', activePlan?.activeLegIndex ?? -1);
    SimVar.SetSimVarValue(
      'L:A339X_DEBUG_FIRST_MISSED_LEG_INDEX',
      'Number',
      activePlan?.firstMissedApproachLegIndex ?? -1,
    );
    SimVar.SetSimVarValue('L:A339X_DEBUG_TYPE_IS_RNAV', 'Bool', approachType === ApproachType.Rnav);
    SimVar.SetSimVarValue('L:A339X_DEBUG_TYPE_IS_GPS', 'Bool', approachType === ApproachType.Gps);

    const unavailableValue = -999999;
    const finiteOrUnavailable = (value: number | undefined): number =>
      typeof value === 'number' && Number.isFinite(value) ? value : unavailableValue;
    let mapLegIndex = -1;
    let mapLegAltitude = unavailableValue;
    let mapLegDistanceToEnd = unavailableValue;

    if (activePlan !== undefined) {
      for (let i = activePlan.firstApproachLegIndex; i < activePlan.firstMissedApproachLegIndex; i++) {
        ...
        if (
          ...
          mapLegIndex = i;
          mapLegAltitude = finiteOrUnavailable(leg.definition.altitude1);
          mapLegDistanceToEnd = finiteOrUnavailable(leg.calculated?.cumulativeDistanceToEndWithTransitions);
          break;
        }
      }
    }

    const destinationLegIndex = activePlan?.destinationLegIndex ?? -1;
    const destinationLeg = destinationLegIndex >= 0 ? activePlan?.maybeElementAt(destinationLegIndex) : undefined;
    const activeLeg = activePlan?.maybeElementAt(activePlan.activeLegIndex);
    const linearDeviation = this.aircraftToDescentProfileRelation.isValid
      ? this.aircraftToDescentProfileRelation.computeLinearDeviation()
      : unavailableValue;

    SimVar.SetSimVarValue(
      'L:A339X_DEBUG_FINAL_ALTITUDE',
      'Feet',
      finiteOrUnavailable(activePlan !== undefined ? this.constraintReader.finalAltitude : undefined),
    );
    SimVar.SetSimVarValue(
      'L:A339X_DEBUG_FINAL_DESCENT_ANGLE',
      'Number',
      finiteOrUnavailable(activePlan !== undefined ? this.constraintReader.finalDescentAngle : undefined),
    );
    SimVar.SetSimVarValue(
      'L:A339X_DEBUG_FAF_DISTANCE_TO_END',
      'Nautical Miles',
      finiteOrUnavailable(activePlan !== undefined ? this.constraintReader.fafDistanceToEnd : undefined),
    );
    SimVar.SetSimVarValue(
      'L:A339X_DEBUG_DISTANCE_TO_END',
      'Nautical Miles',
      finiteOrUnavailable(activePlan !== undefined ? this.constraintReader.distanceToEnd : undefined),
    );
    SimVar.SetSimVarValue('L:A339X_DEBUG_VNAV_LINEAR_DEVIATION', 'Feet', finiteOrUnavailable(linearDeviation));
    SimVar.SetSimVarValue(
      'L:A339X_DEBUG_FG_TARGET_ALTITUDE',
      'Feet',
      finiteOrUnavailable(SimVar.GetSimVarValue('L:A32NX_FG_TARGET_ALTITUDE', 'Feet')),
    );
    SimVar.SetSimVarValue(
      'L:A339X_DEBUG_FG_TARGET_VERTICAL_SPEED',
      'Number',
      finiteOrUnavailable(SimVar.GetSimVarValue('L:A32NX_FG_TARGET_VERTICAL_SPEED', 'Number')),
    );
    SimVar.SetSimVarValue(
      'L:A339X_DEBUG_FG_REQUESTED_VERTICAL_MODE',
      'Enum',
      finiteOrUnavailable(SimVar.GetSimVarValue('L:A32NX_FG_REQUESTED_VERTICAL_MODE', 'Enum')),
    );
    SimVar.SetSimVarValue('L:A339X_DEBUG_MAP_LEG_INDEX', 'Number', mapLegIndex);
    SimVar.SetSimVarValue('L:A339X_DEBUG_MAP_LEG_ALTITUDE_1', 'Feet', mapLegAltitude);
    SimVar.SetSimVarValue('L:A339X_DEBUG_MAP_LEG_CUMULATIVE_DISTANCE_TO_END', 'Nautical Miles', mapLegDistanceToEnd);
    SimVar.SetSimVarValue('L:A339X_DEBUG_DESTINATION_LEG_INDEX', 'Number', destinationLegIndex);
    SimVar.SetSimVarValue(
      'L:A339X_DEBUG_DESTINATION_LEG_VERTICAL_ANGLE',
      'Number',
      destinationLeg?.isDiscontinuity === false
        ? finiteOrUnavailable(destinationLeg.definition.verticalAngle)
        : unavailableValue,
    );
    SimVar.SetSimVarValue(
      'L:A339X_DEBUG_DESTINATION_RUNWAY_THRESHOLD_ELEVATION',
      'Feet',
      finiteOrUnavailable(activePlan?.destinationRunway?.thresholdLocation.alt),
    );
    SimVar.SetSimVarValue(
      'L:A339X_DEBUG_ACTIVE_LEG_CUMULATIVE_DISTANCE_TO_END',
      'Nautical Miles',
      activeLeg?.isDiscontinuity === false
        ? finiteOrUnavailable(activeLeg.calculated?.cumulativeDistanceToEndWithTransitions)
        : unavailableValue,
    );

    const captureDiagnostics =
      this.descentGuidance instanceof DescentGuidance ? this.descentGuidance.getPathCaptureDiagnostics() : undefined;
    SimVar.SetSimVarValue(
      'L:A339X_DEBUG_ACTUAL_VERTICAL_SPEED',
      'Feet per minute',
      finiteOrUnavailable(captureDiagnostics?.actualVerticalSpeed ?? undefined),
    );
    SimVar.SetSimVarValue(
      'L:A339X_DEBUG_CAPTURE_TARGET_VERTICAL_SPEED',
      'Feet per minute',
      finiteOrUnavailable(captureDiagnostics?.targetVerticalSpeed),
    );
    SimVar.SetSimVarValue(
      'L:A339X_DEBUG_VPATH_CAPTURE_THRESHOLD',
      'Feet',
      finiteOrUnavailable(captureDiagnostics?.captureThreshold),
    );
    SimVar.SetSimVarValue(
      'L:A339X_DEBUG_VPATH_CAPTURE_MARGIN',
      'Feet',
      finiteOrUnavailable(captureDiagnostics?.captureMargin),
    );

    if (activePlan === undefined || flightPhase === FmgcFlightPhase.GoAround) {
      this.publishFinalApproachGuidance(approachFamily, false, false);
      this.clearInvalidArmedFinalPrefill(false);
      return;
    }

    const geometryIsFinite =
      Number.isFinite(this.constraintReader.fafDistanceToEnd) &&
      Number.isFinite(this.constraintReader.finalDescentAngle) &&
      Number.isFinite(this.constraintReader.finalAltitude);
    const pathCaptureEligible =
      this.descentGuidance instanceof DescentGuidance && this.descentGuidance.isPathCaptureEligible();
    const availability = getFinalApproachGuidanceAvailability(
      activePlan,
      this.descentProfile?.isReadyToDisplay === true && geometryIsFinite,
      pathCaptureEligible,
      flightPhase === FmgcFlightPhase.Approach,
    );

    this.publishFinalApproachGuidance(
      availability.approachFamily,
      availability.finalCanEngage,
      availability.finalSustainValid,
    );
    this.clearInvalidArmedFinalPrefill(availability.finalSustainValid);
  }

  private publishFinalApproachGuidance(
    approachFamily: FmgcApproachFamily,
    finalCanEngage: boolean,
    finalSustainValid: boolean,
  ): void {
    SimVar.SetSimVarValue('L:A32NX_FG_APPROACH_FAMILY', 'Enum', approachFamily);
    SimVar.SetSimVarValue('L:A32NX_FG_RNAV_APP_SELECTED', 'Bool', approachFamily === FmgcApproachFamily.Rnav);
    SimVar.SetSimVarValue('L:A32NX_FG_FINAL_SUSTAIN_VALID', 'Bool', finalSustainValid);
    SimVar.SetSimVarValue('L:A32NX_FG_FINAL_CAN_ENGAGE', 'Bool', finalCanEngage);
  }

  private clearInvalidArmedFinalPrefill(finalSustainValid: boolean): void {
    const { fcuVerticalMode, fcuArmedVerticalMode } = this.computationParametersObserver.get();
    const isFinalArmed = isArmed(fcuArmedVerticalMode, ArmedVerticalMode.FINAL);

    if (
      !finalSustainValid &&
      isFinalArmed &&
      fcuVerticalMode !== VerticalMode.DES &&
      fcuVerticalMode !== VerticalMode.FINAL
    ) {
      // Armed FINAL is not a VPATH consumer. Once its persistent prerequisites are
      // gone, discard the prefill. Active FINAL deliberately retains its last live
      // sample until the generated release is visible through fcuVerticalMode.
      this.descentGuidance.reset();
    }
  }

  private invalidateIncoherentPlanProfile(): void {
    // FINAL invalidity has already been published. The generated FMGC resets the
    // FINAL latch before selecting its longitudinal law from this input sample.
    SimVar.SetSimVarValue('L:A32NX_FM_VERTICAL_PROFILE_AVAIL', 'Bool', false);
    this.prevMcduPredReadyToDisplay = false;
    this.profileManager.descentProfile?.invalidate();
    this.profileManager.mcduProfile?.invalidate();
    this.aircraftToDescentProfileRelation.reset();
    this.descentGuidance.reset();
  }

  private releaseIncoherentPlanProfile(approachFamily: FmgcApproachFamily, canRebuild: boolean): void {
    this.publishFinalApproachGuidance(approachFamily, false, false);
    this.planProfileCoherence.reset();
    this.invalidateIncoherentPlanProfile();
    this.planProfileReleasePending = false;
    this.planProfileRebuildPermitted = canRebuild;
    this.requestDescentProfileRecomputation ||= canRebuild;
  }

  recompute(geometry: Geometry): void {
    // Fuel predictions and approach/configuration speeds are cached outside VNAV.
    // An active-plan replacement invalidates those caches independently, so no
    // profile may take ownership until the MCDU confirms the matching refresh.
    if (this.readyActivePlanDerivedDataGeneration !== this.activePlanDerivedDataGeneration) {
      this.requestDescentProfileRecomputation = true;
      return;
    }

    const activePlan = this.flightPlanService.active;
    const profileHadOwner = this.planProfileCoherence.hasProfileOwner();
    const profileWasCoherent = this.planProfileCoherence.isCoherent(activePlan, geometry);

    // GuidanceController invokes this periodic callback before update(). If it is
    // the first observer of a changed owner, defer the rebuild so update() can
    // publish one complete FINAL-release sample before new targets are eligible.
    if (
      !profileWasCoherent &&
      (this.planProfileReleasePending || (profileHadOwner && !this.planProfileRebuildPermitted))
    ) {
      this.planProfileReleasePending = true;
      this.requestDescentProfileRecomputation = true;
      return;
    }
    ...
    if (
      !profileWasCoherent ||
      this.shouldUpdateDescentProfile(newParameters, newLegs) ||
      this.requestDescentProfileRecomputation
    ) {
      ...
      this.planProfileCoherence.markRebuilt(activePlan, geometry);
      this.planProfileRebuildPermitted = false;
    }
  ...
  private reset() {
    const activePlan = this.flightPlanService.hasActive ? this.flightPlanService.active : undefined;
    this.publishFinalApproachGuidance(classifyFmgcApproachFamily(activePlan), false, false);
    SimVar.SetSimVarValue('L:A32NX_FM_VERTICAL_PROFILE_AVAIL', 'Bool', false);
    this.descentGuidance.reset();
    this.planProfileCoherence.reset();
    this.planProfileReleasePending = false;
  ...
  private shouldUpdateDescentProfile(
    ...
    return (
      ...
      numberChangedByAtLeast(
        this.lastParameters.estimatedDestinationFuel,
        newParameters.estimatedDestinationFuel,
        DESTINATION_FUEL_RECOMPUTE_THRESHOLD,
      ) ||
      numberChangedByAtLeast(
        this.lastParameters.approachSpeed,
        newParameters.approachSpeed,
        APPROACH_SPEED_RECOMPUTE_THRESHOLD,
      ) ||
      numberChangedByAtLeast(
        this.lastParameters.flapRetractionSpeed,
        newParameters.flapRetractionSpeed,
        APPROACH_SPEED_RECOMPUTE_THRESHOLD,
      ) ||
      numberChangedByAtLeast(
        this.lastParameters.slatRetractionSpeed,
        newParameters.slatRetractionSpeed,
        APPROACH_SPEED_RECOMPUTE_THRESHOLD,
      ) ||
      numberChangedByAtLeast(
        this.lastParameters.cleanSpeed,
        newParameters.cleanSpeed,
        APPROACH_SPEED_RECOMPUTE_THRESHOLD,
      ) ||
  ...
  private didLegsChange(oldLegs: Map<number, Leg>, newLegs: Map<number, Leg>): boolean {
    return didVnavGeometryLegsChange(oldLegs, newLegs, this.guidanceController.activeLegIndex);
  }
  ...
  public invalidateFlightPlanProfile(): number {
    const derivedDataGeneration = ++this.activePlanDerivedDataGeneration;
    const activePlan = this.flightPlanService.hasActive ? this.flightPlanService.active : undefined;
    this.publishFinalApproachGuidance(classifyFmgcApproachFamily(activePlan), false, false);
    this.requestDescentProfileRecomputation = true;
    // Active-plan replacement is an authoritative profile-ownership boundary.
    // The replacement can have the same local version, route, and performance
    // values while plan-derived observer/cache state is still being rebound.
    this.planProfileReleasePending ||= this.planProfileCoherence.hasProfileOwner();
    this.planProfileRebuildPermitted = false;
    this.planProfileCoherence.reset();
    ...
    return derivedDataGeneration;
  }

  /**
   * Completes an active-plan ownership boundary after every plan-dependent
   * derived input consumed by VNAV has been synchronously refreshed.
   */
  public completeActivePlanDerivedDataRefresh(generation: number): void {
    if (generation !== this.activePlanDerivedDataGeneration) {
      return;
    }

    this.readyActivePlanDerivedDataGeneration = generation;
    this.requestDescentProfileRecomputation = true;
  }
