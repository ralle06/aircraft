...
export interface PathCaptureDiagnostics {
  actualVerticalSpeed: FeetPerMinute | null;
  targetVerticalSpeed: FeetPerMinute;
  captureThreshold: Feet;
  /** Positive inside the capture envelope; negative outside it. */
  captureMargin: Feet;
}
...
export class DescentGuidance {
  ...
  update(deltaTime: number, distanceToEnd: NauticalMiles) {
    ...
    const { fcuVerticalMode, fcuArmedVerticalMode } = this.observer.get();
    const isDesModeActive = fcuVerticalMode === VerticalMode.DES;
    const isFinalModeActive = fcuVerticalMode === VerticalMode.FINAL;
    const isFinalModeArmed = isArmed(fcuArmedVerticalMode, ArmedVerticalMode.FINAL);
    const shouldUpdateVpathGuidanceTargets = isDesModeActive || isFinalModeArmed || isFinalModeActive;

    if (isDesModeActive !== (this.verticalState === DescentVerticalGuidanceState.ProvidingGuidance)) {
      this.changeState(
    ...
    if (shouldUpdateVpathGuidanceTargets) {
      ...
      this.updateVpathGuidanceTargets(linearDeviation, targetVerticalSpeed);

      if (isDesModeActive) {
        this.updateDesModeGuidance(linearDeviation, targetVerticalSpeed);
      } else {
        this.requestedVerticalMode = RequestedVerticalMode.None;
      }
    } else {
      this.clearVpathGuidanceTargets();
    }
  ...
  private updateVpathGuidanceTargets(linearDeviation: Feet, targetVerticalSpeed: FeetPerMinute): void {
  ...
  private clearVpathGuidanceTargets(): void {
    this.requestedVerticalMode = RequestedVerticalMode.None;
    this.targetAltitudeGuidance = 0;
    this.targetVerticalSpeed = 0;
  }

  private updateDesModeGuidance(linearDeviation: Feet, targetVerticalSpeed: FeetPerMinute) {
  ...
  /**
   * Whether the current vertical profile is inside the existing VPATH capture envelope.
   *
   * This deliberately delegates to the same condition used by DES rather than defining
   * a second set of FINAL capture thresholds.
   */
  public isPathCaptureEligible(): boolean {
    if (!this.aircraftToDescentProfileRelation.isValid) {
      return false;
    }

    return this.isPathCaptureConditionMet(
      this.aircraftToDescentProfileRelation.computeLinearDeviation(),
      this.aircraftToDescentProfileRelation.currentTargetVerticalSpeed(),
      this.pathCaptureProfile.pathCaptureGain,
    );
  }

  /** Values used by the existing VPATH capture equation, exposed only for temporary runtime diagnostics. */
  public getPathCaptureDiagnostics(): PathCaptureDiagnostics | undefined {
    if (!this.aircraftToDescentProfileRelation.isValid) {
      return undefined;
    }

    const actualVerticalSpeed = this.getVerticalSpeed();
    const targetVerticalSpeed = this.aircraftToDescentProfileRelation.currentTargetVerticalSpeed();
    const linearDeviation = this.aircraftToDescentProfileRelation.computeLinearDeviation();
    const captureThreshold = this.getPathCaptureThreshold(
      actualVerticalSpeed,
      targetVerticalSpeed,
      this.pathCaptureProfile.pathCaptureGain,
    );
    ...
    return {
      ...
      targetVerticalSpeed,
      captureThreshold,
      captureMargin: captureThreshold - Math.abs(linearDeviation),
    };
  }
  ...
  private isPathCaptureConditionMet(linearDeviation: Feet, targetVerticalSpeed: FeetPerMinute, gain: number): boolean {
    ...
    return Math.abs(linearDeviation) < this.getPathCaptureThreshold(verticalSpeed, targetVerticalSpeed, gain);
  }

  private getPathCaptureThreshold(
    verticalSpeed: FeetPerMinute | null,
    targetVerticalSpeed: FeetPerMinute,
    gain: number,
  ): Feet {
    if (verticalSpeed === null) {
      return this.pathCaptureProfile.fallbackPathCaptureDeviation;
    }
